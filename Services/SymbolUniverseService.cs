...
{
    /// <summary>
    /// Builds the symbol universe from a portfolio data source.
    /// File discovery, symbol identity extraction, and summary enrichment live here;
    /// chart-bar loading is intentionally handled by MarketBarDataService.
    /// </summary>
    internal sealed class SymbolUniverseService
    {
        ...
        public Task<List<SymbolInfo>> GetSymbolsAsync(
            ...
            Task.Run(() => GetSymbols(portfolio, cancellationToken), cancellationToken);
        ...
        {
            if (!Directory.Exists(folderPath) || portfolio.Symbols == null || portfolio.Symbols.Count == 0)
                return new List<SymbolInfo>();

            List<string> paths = portfolio.Symbols
            ...
            Parallel.ForEach(paths, options, filePath =>
            {
                cancellationToken.ThrowIfCancellationRequested();
            ...
            CancellationToken cancellationToken)
        {
            ...
            Parallel.ForEach(files, options, file =>
            {
                ...
                try
                {
                    SymbolInfo? info = CreateSymbolInfoFromFile(file, dataSource, cancellationToken);
                    if (info != null) parsed.Add(info);
                }
                catch (OperationCanceledException)
                {
            ...
            CancellationToken cancellationToken)
        {
            ...
            MarketBar? firstBar = summary.FirstBar;
            MarketBar? lastBar = summary.LastBar;
            string contentSymbol = firstBar != null && !string.IsNullOrWhiteSpace(firstBar.PersianTicker)
                ? firstBar.PersianTicker
                : firstBar?.EnglishTicker ?? "";

            if (string.IsNullOrWhiteSpace(contentSymbol))
                contentSymbol = Path.GetFileNameWithoutExtension(filePath);

            var contentInfo = new SymbolInfo
            {
                Symbol = contentSymbol,
                DisplayName = firstBar != null && !string.IsNullOrWhiteSpace(firstBar.PersianTicker)
                    ? firstBar.PersianTicker
                    : contentSymbol,
                FilePath = filePath,
                IsSelected = false
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                cancellationToken.ThrowIfCancellationRequested();
                var summary = ParseSummary(filePath, dataSource, cancellationToken);
                ApplyLastBarInfo(symbolInfo, summary.LastBar);
            }
            catch (OperationCanceledException)
            {
                throw;
        ...
        private static (MarketBar? FirstBar, MarketBar? LastBar) ParseSummary(
            string filePath,
            DataSource dataSource,
            CancellationToken cancellationToken)
        {
