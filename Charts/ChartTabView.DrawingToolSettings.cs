...
namespace TradeIt.Charts
{
    public partial class ChartTabView
    {
        ...
        {
            EventManager.RegisterClassHandler(typeof(ChartTabView), FrameworkElement.LoadedEvent, new RoutedEventHandler(DrawingToolSettings_Loaded));
            return true;
        }
        private static void DrawingToolSettings_Loaded(object sender, RoutedEventArgs e) { if (sender is ChartTabView chart) chart.AttachDrawingToolSettingsMenus(); }
        ...
        {
            DrawingToolStyle style = GetDrawingToolStyle(key); ScottPlot.Color color = ScottPlot.Color.FromHtml(style.Color); ScottPlot.LinePattern pattern = GetDrawingLinePattern(style.LineStyle);
            switch (key)
            {
                ...
                case "FibonacciRetracement": case "FibonacciExtension": foreach (var d in _fibonacciDrawings) { if ((key == "FibonacciExtension") != d.IsExtension) continue; foreach (var line in d.Lines) { line.LineColor = color; line.LineWidth = (float)style.LineWidth; line.LinePattern = pattern; } foreach (var label in d.Labels) { label.LabelFontColor = color; label.LabelBorderColor = color; } } break;
                case "Text": foreach (var d in _textDrawings) if (d.PlotText != null) { d.PlotText.LabelFontColor = color; d.PlotText.LabelBorderColor = color; } break;
            }
