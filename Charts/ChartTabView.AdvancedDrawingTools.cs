...
namespace TradeIt.Charts
{
    public partial class ChartTabView
    {
        ...
        private sealed class ParallelChannelDrawing { public ScottPlot.Coordinates A { get; set; } public ScottPlot.Coordinates B { get; set; } public ScottPlot.Coordinates C { get; set; } public ScottPlot.Plottables.Scatter? BaseLine { get; set; } public ScottPlot.Plottables.Scatter? ParallelLine { get; set; } }
        private sealed class RectangleDrawing { public ScottPlot.Coordinates A { get; set; } public ScottPlot.Coordinates B { get; set; } public readonly List<ScottPlot.Plottables.Scatter> Lines = new(); }
        private sealed class PitchforkDrawing { public ScottPlot.Coordinates A { get; set; } public ScottPlot.Coordinates B { get; set; } public ScottPlot.Coordinates C { get; set; } public ScottPlot.Plottables.Scatter? MedianLine { get; set; } public ScottPlot.Plottables.Scatter? UpperLine { get; set; } public ScottPlot.Plottables.Scatter? LowerLine { get; set; } }
        ...
        private void AdvancedDrawing_MouseDown(object sender, WpfMouseButtonEventArgs e)
        {
            ...
            if (_advancedDrawingP1 == null) { _advancedDrawingP1 = point; ChartInfoTextBlock.Text = (int)_activeDrawingTool == AdvancedToolParallelChannel ? $"{_symbol.Symbol} | کانال موازی: نقطه اول خط پایه انتخاب شد" : $"{_symbol.Symbol} | چنگال اندروز: نقطه A انتخاب شد؛ نقطه B را کلیک کنید"; e.Handled = true; return; }
            if (_advancedDrawingP2 == null) { _advancedDrawingP2 = point; ChartInfoTextBlock.Text = (int)_activeDrawingTool == AdvancedToolParallelChannel ? $"{_symbol.Symbol} | کانال موازی: نقطه دوم خط پایه انتخاب شد؛ نقطه سوم را برای فاصله کلیک کنید" : $"{_symbol.Symbol} | چنگال اندروز: نقطه B انتخاب شد؛ نقطه C را کلیک کنید"; e.Handled = true; return; }
            if ((int)_activeDrawingTool == AdvancedToolParallelChannel) { var d = new ParallelChannelDrawing { A = _advancedDrawingP1.Value, B = _advancedDrawingP2.Value, C = point }; _parallelChannels.Add(d); AddParallelChannelToChart(d); ChartInfoTextBlock.Text = $"{_symbol.Symbol} | کانال موازی رسم شد"; }
            else { var d = new PitchforkDrawing { A = _advancedDrawingP1.Value, B = _advancedDrawingP2.Value, C = point }; _pitchforks.Add(d); AddPitchforkToChart(d); ChartInfoTextBlock.Text = $"{_symbol.Symbol} | چنگال اندروز رسم شد"; }
        ...
        private void AdvancedDrawing_MouseMove(object sender, WpfMouseEventArgs e)
        {
            ...
            if ((int)_activeDrawingTool == AdvancedToolRectangle && _advancedDrawingP1 != null) { double left = Math.Min(_advancedDrawingP1.Value.X, point.X), right = Math.Max(_advancedDrawingP1.Value.X, point.X), bottom = Math.Min(_advancedDrawingP1.Value.Y, point.Y), top = Math.Max(_advancedDrawingP1.Value.Y, point.Y); _advancedDrawingPreview1 = AddScatterLine(left, bottom, right, bottom); _advancedDrawingPreview2 = AddScatterLine(left, top, right, top); }
            else if ((int)_activeDrawingTool == AdvancedToolParallelChannel && _advancedDrawingP1 != null) { if (_advancedDrawingP2 == null) _advancedDrawingPreview1 = AddScatterLine(_advancedDrawingP1.Value.X, _advancedDrawingP1.Value.Y, point.X, point.Y); else { _advancedDrawingPreview1 = AddScatterLine(_advancedDrawingP1.Value.X, _advancedDrawingP1.Value.Y, _advancedDrawingP2.Value.X, _advancedDrawingP2.Value.Y); _advancedDrawingPreview2 = AddParallelLineSegment(_advancedDrawingP1.Value, _advancedDrawingP2.Value, point); } }
            else if ((int)_activeDrawingTool == AdvancedToolPitchfork && _advancedDrawingP1 != null) { if (_advancedDrawingP2 == null) _advancedDrawingPreview1 = AddScatterLine(_advancedDrawingP1.Value.X, _advancedDrawingP1.Value.Y, point.X, point.Y); else { var target = Midpoint(_advancedDrawingP2.Value, point); _advancedDrawingPreview1 = AddRayThroughPoints(_advancedDrawingP1.Value, target); _advancedDrawingPreview2 = AddParallelRayThroughPoint(_advancedDrawingP1.Value, target, _advancedDrawingP2.Value); } }
            Chart.Refresh();
        }
        ...
        private void AddRectangleToChart(RectangleDrawing d) { double left = Math.Min(d.A.X, d.B.X), right = Math.Max(d.A.X, d.B.X), bottom = Math.Min(d.A.Y, d.B.Y), top = Math.Max(d.A.Y, d.B.Y); d.Lines.Add(AddScatterLine(left, bottom, right, bottom)); d.Lines.Add(AddScatterLine(right, bottom, right, top)); d.Lines.Add(AddScatterLine(right, top, left, top)); d.Lines.Add(AddScatterLine(left, top, left, bottom)); }
