...
namespace TradeIt.Charts
{
    public partial class ChartTabView
    {
        ...
        {
            EventManager.RegisterClassHandler(typeof(ChartTabView), FrameworkElement.LoadedEvent, new RoutedEventHandler(VolumeSyncFix_Loaded));
            return true;
        }

        private static void VolumeSyncFix_Loaded(object sender, RoutedEventArgs e)
        {
            if (sender is not ChartTabView chart || chart._volumeSyncFixLoaded) return;
            ...
            chart.Chart.PreviewMouseWheel += chart.VolumeSyncFix_MainChartInteractionFinished;

            foreach (System.Windows.Controls.Button button in chart.FindVisualChildren<System.Windows.Controls.Button>())
            {
            ...
            chart.Dispatcher.BeginInvoke(DispatcherPriority.Render, new Action(chart.RefreshVolumeFollower));
        }

        private void VolumeSyncFix_MainChartInteractionFinished(object? sender, RoutedEventArgs e)
        {
            Dispatcher.BeginInvoke(DispatcherPriority.Render, new Action(RefreshVolumeFollower));
        }

        private void VolumeSyncFix_MainChartInteractionFinished(object? sender, System.Windows.Input.MouseWheelEventArgs e)
        {
            Dispatcher.BeginInvoke(DispatcherPriority.Render, new Action(RefreshVolumeFollower));
        }
        ...
        private void RefreshVolumeFollower()
        {
            ...
            if (maxVisibleVolume <= 0)
            {
                ...
                if (double.IsFinite(volume) && volume > 0) maxVisibleVolume = volume;
            }
        ...
        private System.Collections.Generic.IEnumerable<T> FindVisualChildren<T>() where T : DependencyObject
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (child is FrameworkElement element)
                    foreach (T descendant in FindVisualChildren<T>(element)) yield return descendant;
            }
        }
