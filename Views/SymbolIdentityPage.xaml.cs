...
{
    public partial class SymbolIdentityPage : UserControl
    {
        ...
        private void DeleteSelectedButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageBox.Show($"{selected.Count:N0} شناسه انتخاب‌شده حذف شود؟", "تأیید حذف", MessageBoxButton.YesNo, MessageBoxImage.Warning) != MessageBoxResult.Yes)
                return;
        ...
        private void DeleteAllButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageBox.Show("تمام اطلاعات شناسه‌ها حذف خواهد شد. این عملیات قابل برگشت نیست. ادامه می‌دهید؟", "حذف کامل", MessageBoxButton.YesNo, MessageBoxImage.Warning) != MessageBoxResult.Yes)
                return;
        ...
        {
            if (IdentitiesDataGrid.SelectedItem is SymbolIdentity item)
                FillEditor(item);
        }
        ...
        private void ImportExcelButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Title = "انتخاب فایل Excel شناسه نمادها",
                Filter = "Excel (*.xlsx)|*.xlsx|همه فایل‌ها (*.*)|*.*",
                Multiselect = false
            };
            ...
            try
            {
                ...
                if (MessageBox.Show(message, "پیش‌نمایش Import", MessageBoxButton.YesNo, MessageBoxImage.Question) != MessageBoxResult.Yes)
                    return;
        ...
        private static List<SymbolIdentity> ReadExcel(string path)
        {
            ...
            string[][] aliases =
            {
                new[] { "کد12رقمی نماد", "کد 12 رقمی نماد", "SymbolId12", "SymbolId" },
                new[] { "کد5رقمی نماد", "کد 5 رقمی نماد", "SymbolCode5", "SymbolCode" },
                new[] { "نام لاتین شرکت", "CompanyNameLatin" },
                new[] { "کد4رقمی شرکت", "کد 4 رقمی شرکت", "CompanyCode4" },
                new[] { "نام شرکت", "CompanyName" },
                new[] { "نماد فارسی", "SymbolNameFa", "نماد" },
                new[] { "نماد30رقمی فارسی", "نماد 30 رقمی فارسی", "SymbolName30Fa" },
                new[] { "کد12رقمی شرکت", "کد 12 رقمی شرکت", "CompanyId12" },
                new[] { "بازار", "Market" },
                new[] { "کد تابلو", "BoardCode" },
                new[] { "کد گروه صنعت", "IndustryCode" },
                new[] { "گروه صنعت", "IndustryName" },
                new[] { "کد زیرگروه صنعت", "کد زیر گروه صنعت", "SubIndustryCode" },
                new[] { "زیرگروه صنعت", "زیر گروه صنعت", "SubIndustryName" }
            };
            ...
            foreach (var row in used.Rows().Skip(1))
            {
                ...
                result.Add(new SymbolIdentity
                {
                    SymbolId12 = id, SymbolCode5 = Value(col[1]), CompanyNameLatin = Value(col[2]), CompanyCode4 = Value(col[3]),
                    CompanyName = Value(col[4]), SymbolNameFa = Value(col[5]), SymbolName30Fa = Value(col[6]), CompanyId12 = Value(col[7]),
                    Market = Value(col[8]), BoardCode = Value(col[9]), IndustryCode = Value(col[10]), IndustryName = Value(col[11]),
                    SubIndustryCode = Value(col[12]), SubIndustryName = Value(col[13])
                });
            }
            return result;
        }
        ...
        private static int FindColumn(Dictionary<string, int> headers, IEnumerable<string> aliases)
        {
            foreach (string alias in aliases)
                if (headers.TryGetValue(NormalizeHeader(alias), out int c)) return c;
            return -1;
        }
        ...
        {
            SymbolId12 = SymbolId12TextBox.Text.Trim(), SymbolCode5 = SymbolCode5TextBox.Text.Trim(), CompanyNameLatin = CompanyNameLatinTextBox.Text.Trim(),
            CompanyCode4 = CompanyCode4TextBox.Text.Trim(), CompanyName = CompanyNameTextBox.Text.Trim(), SymbolNameFa = SymbolNameFaTextBox.Text.Trim(),
            SymbolName30Fa = SymbolName30FaTextBox.Text.Trim(), CompanyId12 = CompanyId12TextBox.Text.Trim(), Market = MarketTextBox.Text.Trim(),
            BoardCode = BoardCodeTextBox.Text.Trim(), IndustryCode = IndustryCodeTextBox.Text.Trim(), IndustryName = IndustryNameTextBox.Text.Trim(),
            SubIndustryCode = SubIndustryCodeTextBox.Text.Trim(), SubIndustryName = SubIndustryNameTextBox.Text.Trim()
        };

        private void FillEditor(SymbolIdentity x)
        {
            _editingId = x.SymbolId12;
            SymbolId12TextBox.Text = x.SymbolId12; SymbolCode5TextBox.Text = x.SymbolCode5; CompanyNameLatinTextBox.Text = x.CompanyNameLatin;
            CompanyCode4TextBox.Text = x.CompanyCode4; CompanyNameTextBox.Text = x.CompanyName; SymbolNameFaTextBox.Text = x.SymbolNameFa;
            SymbolName30FaTextBox.Text = x.SymbolName30Fa; CompanyId12TextBox.Text = x.CompanyId12; MarketTextBox.Text = x.Market;
            BoardCodeTextBox.Text = x.BoardCode; IndustryCodeTextBox.Text = x.IndustryCode; IndustryNameTextBox.Text = x.IndustryName;
            SubIndustryCodeTextBox.Text = x.SubIndustryCode; SubIndustryNameTextBox.Text = x.SubIndustryName;
        }
