...
namespace TradeIt
{
    public partial class MainWindow
    {
        ...
        {
            if (_symbolFiltersInitialized) return;
            if (SymbolFilterHost == null) return;
            _symbolFiltersInitialized = true;
            Grid filterHostGrid = BuildFilterHost();
            SymbolFilterHost.Children.Clear();
            SymbolFilterHost.Children.Add(filterHostGrid);
            LoadFilterDefaults();
        ...
        private Grid BuildFilterHost()
        {
            ...
            var title = new TextBlock { Text = "همه فیلترها با AND", FontWeight = FontWeights.SemiBold, Margin = new Thickness(0, 0, 0, 4) };
            ...
            _tradeStatusFilterComboBox = new WpfComboBox { Height = 27 };
        ...
        private FrameworkElement BuildPriceFilterRow(int index)
        {
            ...
            var comparison = new WpfComboBox { Width = 55, Height = 26, Margin = new Thickness(3, 0, 3, 0) }; AddComboItems(comparison, (NumericComparison.GreaterThan, ">"), (NumericComparison.Equal, "="), (NumericComparison.NotEqual, "≠"), (NumericComparison.LessThan, "<"));
        ...
        private async Task ApplyAllSymbolFiltersAsync()
        {
            ...
            try
            {
                ...
                IEnumerable<SymbolInfo> query = _allSymbols; SymbolFilterSettings settings = _symbolFilterSettings;
        ...
        {
            _symbolFilterSettings.TradeStatus = GetComboValue<TradeStatusFilter>(_tradeStatusFilterComboBox!); _symbolFilterSettings.NameFilter = GetComboValue<SymbolNameFilter>(_nameFilterComboBox!); _symbolFilterSettings.NameText = _nameFilterTextBox!.Text.Trim(); _symbolFilterSettings.DaysWithoutTradeEnabled = _daysWithoutTradeCheckBox!.IsChecked == true; _symbolFilterSettings.DaysWithoutTrade = ParsePositiveInt(_daysWithoutTradeTextBox!.Text, 0); _symbolFilterSettings.DaysWithTradeEnabled = _daysWithTradeCheckBox!.IsChecked == true; _symbolFilterSettings.DaysWithTrade = ParsePositiveInt(_daysWithTradeTextBox!.Text, 0); _symbolFilterSettings.VolumeFilterEnabled = _volumeFilterCheckBox!.IsChecked == true; _symbolFilterSettings.VolumeAverageDays = ParsePositiveInt(_volumeAverageDaysTextBox!.Text, 20); _symbolFilterSettings.VolumeMultiplier = ParsePositiveDouble(_volumeMultiplierTextBox!.Text, 2.0);
        ...
        private static bool MatchName(string value, string text, SymbolNameFilter filter) { if (filter == SymbolNameFilter.All) return true; if (filter == SymbolNameFilter.Contains) return value.Contains(text, StringComparison.OrdinalIgnoreCase); if (filter == SymbolNameFilter.StartsWith) return value.StartsWith(text, StringComparison.OrdinalIgnoreCase); if (filter == SymbolNameFilter.EndsWith) return value.EndsWith(text, StringComparison.OrdinalIgnoreCase); if (filter == SymbolNameFilter.DoesNotContain) return !value.Contains(text, StringComparison.OrdinalIgnoreCase); int index = value.IndexOf(text, StringComparison.OrdinalIgnoreCase); return index > 0 && index + text.Length < value.Length; }
        private async Task EnsureBarsLoadedAsync(List<SymbolInfo> symbols, CancellationToken token) { if (_selectedPortfolio == null) return; List<SymbolInfo> missing = symbols.Where(x => !_symbolFilterBars.ContainsKey(x.FilePath)).ToList(); if (missing.Count == 0) return; await Task.Run(() => { foreach (SymbolInfo symbol in missing) { token.ThrowIfCancellationRequested(); List<MarketBar> bars = _symbolDataService.LoadBars(symbol, _selectedPortfolio); lock (_symbolFilterBars) _symbolFilterBars[symbol.FilePath] = bars.OrderBy(x => x.Timestamp ?? DateTime.MinValue).ToList(); } }, token); }
        private bool PassHistoricalFilters(SymbolInfo symbol, SymbolFilterSettings settings) { if (!_symbolFilterBars.TryGetValue(symbol.FilePath, out List<MarketBar>? bars) || bars.Count == 0) return false; if (settings.VolumeFilterEnabled) { int n = Math.Max(1, settings.VolumeAverageDays); if (bars.Count < n) return false; double average = bars.TakeLast(n).Average(x => x.Volume); if (bars[^1].Volume < average * settings.VolumeMultiplier) return false; } foreach (PriceFilter filter in settings.PriceFilters) { if (!filter.Enabled) continue; int days = Math.Max(1, filter.Days); if (bars.Count <= days) return false; double latest = GetPriceField(bars[^1], filter.Field); double previous = GetPriceField(bars[bars.Count - 1 - days], filter.Field); if (!Compare(latest, previous, filter.Comparison)) return false; } return true; }
        ...
        private void RenumberFilteredSymbols(List<SymbolInfo> symbols) { for (int i = 0; i < symbols.Count; i++) symbols[i].RowNumber = i + 1; }
    }
}