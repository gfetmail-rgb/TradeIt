...
namespace TradeIt
{
    public partial class MainWindow
    {
        ...
        {
            foreach (var item in items) combo.Items.Add(new FilterComboItem<T>(item.Value, item.Text));
        ...
        private void LoadFilterDefaults()
        {
            ...
            foreach (var row in _priceFilterControls) row.Enabled.IsChecked = false;
        }

        private void SymbolFilterInputChanged(object? sender, RoutedEventArgs e)
        {
            if (!_symbolFiltersApplying) _ = ApplyAllSymbolFiltersAsync();
        }
        ...
        private async Task ApplyAllSymbolFiltersAsync()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(search))
                    query = query.Where(x => x.Symbol.Contains(search, StringComparison.OrdinalIgnoreCase) || x.DisplayName.Contains(search, StringComparison.OrdinalIgnoreCase));
                ...
                if (_symbolFilterStatusTextBlock != null) _symbolFilterStatusTextBlock.Text = $"نتیجه: {candidates.Count:N0} نماد";
            }
            catch (OperationCanceledException) { }
            catch (Exception ex)
            {
                if (_symbolFilterStatusTextBlock != null) _symbolFilterStatusTextBlock.Text = $"خطا در فیلتر: {ex.Message}";
            }
        }
        ...
        private async Task EnsureBarsLoadedAsync(List<SymbolInfo> symbols, CancellationToken token)
        {
            ...
            await Task.Run(() =>
            {
                foreach (SymbolInfo symbol in missing)
                {
                    ...
                    lock (_symbolFilterBars) _symbolFilterBars[symbol.FilePath] = bars.OrderBy(x => x.Timestamp ?? DateTime.MinValue).ToList();
                }
            }, token);
        }
        ...
        private static void RenumberFilteredSymbols(List<SymbolInfo> symbols)
        {
            for (int i = 0; i < symbols.Count; i++) symbols[i].RowNumber = i + 1;
        }
    }
}